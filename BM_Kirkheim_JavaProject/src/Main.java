...
public class Main {
    ...
    private static final String BOOLMODE = "AND";
    ...
    //constant to indicate whether matrices for all possible BNs should be printed
    private static final boolean PRINTALLMATRICES = false;
    //debug flag for verbose iterations
    private static final boolean DEBUGFLAG = false;

    public static void main(String[] args){
        //iterate through all BNs, encoded as elaborated in the code documentation
        for(int index=0;index<243;index++){
            if(PRINTALLMATRICES) System.out.println("-------------Case index "+index+"-------------");
            //print BN if it fits the experimental data and is thus a candidate network
            if(testIndexInMedia(index)){
                System.out.println("Model corresponding to index "+index+" adheres to experiment!");
                System.out.println("Model matrix is: ");
                printMatrix(fillMatrix(index));
            //print BN if all matrices should be printed
            }else if(PRINTALLMATRICES){
                System.out.println("No problem with matrix ");
                printMatrix(fillMatrix(index));
            }
        }
    }
    ...
    /**
     * Helper function giving false for intToCase = 0, true else
     *
     * @param intToCase integer to cast to boolean
     * @return false if intToCase is 0, true otherwise
     */
    private static boolean intToBool(int intToCase) {
        if(intToCase == 0) return false;
        else return true;
    }
    ...
    /**
     * A helper function to test whether two expression profiles are equal or different.
     * Two expression profiles are seen as equal when all TF expressions are equal (factors expressed in one are expressed in the other, factors not expressed are not either)
     *
     * @param tFexpressionOld HashMap indicating whether the transcription factors are expressed or not
     * @param tFexpressionNew HashMap indicating whether the transcription factors are expressed or not
     * @return true if the expression profiles given as arguments are identical, false otherwise
     */
    private static boolean TFExpressionDifferent(HashMap<String, Boolean> tFexpressionOld, HashMap<String, Boolean> tFexpressionNew) {
        if(!DEBUGFLAG) return !(((tFexpressionOld.get("Dlx5")) == (tFexpressionNew.get("Dlx5"))) && ((tFexpressionOld.get("Msx2")) == (tFexpressionNew.get("Msx2"))) && ((tFexpressionOld.get("Runx2")) == (tFexpressionNew.get("Runx2"))));
        /*else{
            System.out.println("Dlx5: "+(tFexpressionOld.get("Dlx5"))+" == "+(tFexpressionNew.get("Dlx5"))+" is "+((tFexpressionOld.get("Dlx5")) == (tFexpressionNew.get("Dlx5"))));
            System.out.println("Msx2: "+(tFexpressionOld.get("Msx2"))+" == "+(tFexpressionNew.get("Msx2"))+" is "+((tFexpressionOld.get("Msx2")) == (tFexpressionNew.get("Msx2"))));
            System.out.println("Runx2: "+(tFexpressionOld.get("Runx2"))+" == "+(tFexpressionNew.get("Runx2"))+" is "+((tFexpressionOld.get("Runx2")) == (tFexpressionNew.get("Runx2"))));
            return !(((tFexpressionOld.get("Dlx5")) == (tFexpressionNew.get("Dlx5"))) && ((tFexpressionOld.get("Msx2")) == (tFexpressionNew.get("Msx2"))) && ((tFexpressionOld.get("Runx2")) == (tFexpressionNew.get("Runx2"))));
        }*/
        return !(((tFexpressionOld.get("Dlx5")) == (tFexpressionNew.get("Dlx5"))) && ((tFexpressionOld.get("Msx2")) == (tFexpressionNew.get("Msx2"))) && ((tFexpressionOld.get("Runx2")) == (tFexpressionNew.get("Runx2"))));
    }

    /**
     * Sets up the expression matrix for the TF as all being inactive (in the beginning of the simulation)
     *
     * @return a TF expression profile where all TFs are set as non-expressed
     */
    private static HashMap<String, Boolean> setupExpressionMatrix(){
        /**
         * Since mRNA levels are low in mES cells, the initial mRNA expression is considered to be off for all species of interest
         */
        HashMap<String, Boolean> TFexpression = new HashMap<>(3);
        TFexpression.put("Dlx5", false);
        TFexpression.put("Msx2", false);
        TFexpression.put("Runx2", false);
        return TFexpression;
    }

    /**
     * Prints a matrix encoding the BN.
     *
     * @param matrixToPrint HashMap of TFs and their influences as indicated in the documentation.
     */
    private static void printMatrix(HashMap<String, int[]> matrixToPrint){
        /**
         * Matrix to keep the influence of species for the three species of investigation (key)
         * Structure of entries is:
         * 0. entry: Influence of BMP2
         ...
         * 4. entry: Influence of Runx2
         */
        for(String tf : matrixToPrint.keySet()){
            System.out.println("\nMatrix entry for "+tf+": ");
            System.out.println("\nInfluence of BMP2: "+matrixToPrint.get(tf)[0]+"\tInfluence of TGFb1: "+matrixToPrint.get(tf)[1]+"\tInfluence of Dlx5: "+matrixToPrint.get(tf)[2]+"\tInfluence of Msx2: "+matrixToPrint.get(tf)[3]+"\tInfluence of Runx2: "+matrixToPrint.get(tf)[4]+"\n");
        }
    }
}
